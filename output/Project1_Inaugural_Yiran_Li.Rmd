...
```{r, warning = FALSE, echo = FALSE, message = FALSE}
#print("FDR's Most Sentimentally Charged Sentences")
speech_df=tbl_df(sentence_list)%>%
  filter(File=="FranklinDRoosevelt", type=="nomin", Term==1, word.count>=5)%>%
  select(sentences, anger:trust)
speech_df=as.data.frame(speech_df)
#as.character(speech_df$sentences[apply(speech_df[,-1], 2, which.max)])
...
trust_ord <- order(speech_df2$trust, decreasing = TRUE)
FDR_reord_speechdf2<-speech_df2$sentences[trust_ord]
#as.character(FDR_reord_speechdf2[1:5])

#

#print("FDR's Most Trust Sentences in Inaugural Speeches")
speech_df2=tbl_df(sentence_list)%>%
  filter(File=="FranklinDRoosevelt", type=="inaug", word.count>=5)%>%
  select(sentences, anger:trust)
speech_df2=as.data.frame(speech_df2)
#as.character(speech_df2$sentences[apply(speech_df2[,-1], 2, which.max)])
trust_ord <- order(speech_df2$trust, decreasing = TRUE)
reord_speechdf2<-speech_df2$sentences[trust_ord]
#as.character(reord_speechdf2[1:5])
```
...
## DT_sentiment
#print("Trump's Most Sentimentally Charged Sentences")
speech_df <- DT_sentiment %>%
  select(sentences, anger:trust)
speech_df=as.data.frame(speech_df)
#as.character(speech_df$sentences[apply(speech_df[,-1], 2, which.max)])
##
...
trust_ord2 <- order(speech_df2$trust, decreasing = TRUE)
DT_reord_speechdf2<-speech_df2$sentences[trust_ord2]
#as.character(DT_reord_speechdf2[1:5])

#print("Trump's Most Trust Sentences in Inaugural Speeches")
speech_df2 <- DT_sentiment%>%
  filter(type=="inaug") %>%
  select(sentences, anger:trust)
speech_df2 <- as.data.frame(speech_df2)
trust_ord2 <- order(speech_df2$trust, decreasing = TRUE)
reord_speechdf3<-speech_df2$sentences[trust_ord2]
#as.character(reord_speechdf3[1:5])
```
